...
def generate_with_matplotlib(
    ...
    for idx, subplot_type in enumerate(subplot_types):
        ...
        if subplot_type != "bar" and inverted_bars[idx]:
            raise ValueError("inverted bar can only be enabled for bar subplots")
    ...
    if x_is_datetime and mdates is not None:
        ...
        if span_seconds < 86400:
            ...
            datetime_rotation = 90
        else:
            axes[-1].xaxis.set_major_formatter(mdates.DateFormatter("%Y-%m-%d"))
            datetime_rotation = 0

    def apply_datetime_tick_rotation() -> None:
        """Apply deterministic x-tick rotation for datetime axes."""
        if not (x_is_datetime and mdates is not None and datetime_rotation is not None):
            return

        for label in axes[-1].get_xticklabels():
            label.set_rotation(datetime_rotation)
            label.set_ha("right" if datetime_rotation else "center")

    bar_widths = _compute_bar_widths(x_values)

    if not animate:
        for idx, axis in enumerate(axes):
            color = palette[idx]
            if subplot_types[idx] == "line":
                axis.plot(x_values, y_series[idx], color=color, linewidth=2.2)
            elif subplot_types[idx] == "scatter":
                axis.scatter(x_values, y_series[idx], color=color, s=24)
            else:
                axis.bar(x_values, y_series[idx], color=color, alpha=0.88, width=bar_widths)
            ...
            if subplot_types[idx] == "bar" and inverted_bars[idx]:
                axis.invert_yaxis()

        if x_is_datetime and mdates is not None and datetime_rotation is not None:
            fig.autofmt_xdate(rotation=datetime_rotation)
    ...
    x_anim = [x_values[idx] for idx in frame_indexes]
    ...
    if x_is_datetime:
        ...
        x_max = max(x_values)
        if x_min == x_max:
            x_max = x_min.replace(microsecond=1)
        for axis in axes:
    ...
    def init() -> tuple[Any]:
        """Initialize empty animated artists for each subplot."""
    ...
    animation = FuncAnimation(
        ...
        init_func=init,
        frames=len(x_anim),
        interval=max(20, int(80 / max(animation_speed, 0.25))),
        blit=False,
    )
    if x_is_datetime and mdates is not None and datetime_rotation is not None:
        fig.autofmt_xdate(rotation=datetime_rotation)
        apply_datetime_tick_rotation()
    fig.tight_layout()
    fps = max(4, min(48, int(round(12 * max(animation_speed, 0.25)))))
